...
class FourElementsPicker extends StatelessWidget {
  const FourElementsPicker({super.key});
      ...
      return Row(
        ...
        children: [
          DropTarget(
            ...
            child: DropContentWidget(file: state.fourElementsState.image1),
          ),
          DropTarget(
            onDragDone: (detail) {
              if (!state.showImportDirectoryAlert) {
            ...
            child: DropContentWidget(file: state.fourElementsState.image2),
          ),
          DropTarget(
            onDragDone: (detail) {
              if (!state.showImportDirectoryAlert) {
            ...
            child: DropContentWidget(file: state.fourElementsState.image3),
          ),
          DropTarget(
            onDragDone: (detail) {
              if (!state.showImportDirectoryAlert) {
            ...
            child: DropContentWidget(file: state.fourElementsState.image4),
          ),
        ],
      );
    });
